...
unsigned long printPeriod = 500; // in milliseconds e.g. 1000 would be 1 second

unsigned long timeElapsed = 0; // Track time in milliseconds since last reading 
...
void loop() {
  ...
  while( true ) {   
    ...
    if((unsigned long)(millis() - timeElapsed) >= printPeriod) { //every second we do the calculation
      timeElapsed = millis();   // update time
